...
function [p_mw, n] = compare_avg_length_complex( tile, bins, events)

% function returns the denisty of events in a SINGLE tile (support)


tile_pos_x=[]; tile_pos_y=[]; tile_rand_x=[]; tile_rand_y=[];tile_weights=[]; tile_rand_weights=[];
for c1=1:length(tile(:,1))
    tile1(c1)=min(tile(c1,:));
    tile2(c1)=max(tile(c1,:));
    pos1=bins(tile1(c1),1:3);
    pos2=bins(tile2(c1),1:3);
    tile_events=list_events(events(:,1:11),bins(tile1(c1),1:3),bins(tile2(c1),1:3),0,0,[],[],[],[]);
    
       rng('default')
   
       tile_pos_x=[tile_pos_x;tile_events(:,2)];
       tile_pos_y=[tile_pos_y;tile_events(:,5)];
       tile_weights=[tile_weights;tile_events(:,11)];
       ...
       tile_rand_weights=[tile_rand_weights;tile_weights(randi(length(tile_weights), 20, 1),:)];
       ...
       for i =1:length(C)-1
            ...
            real_dist(i,1)=real_dist(i,1)*(tile_weights(a,:)*tile_weights(b,:));
       ...
       for i =1:length(C1)-1
            ...
            randi(length(tile_weights));
            rand_dist(i,1)=pdist(rand_set,'euclidean');
            rand_dist(i,1)=rand_dist(i,1)*(tile_rand_weights(a,:)*tile_rand_weights(b,:));

            
       end
       
       histogram(rand_dist, 10)
