...
notes= [newline, 'warning: if this takes a long time, try changing the FDR threshold or the length of your bins.', ...
        newline, 'This function loops through every gene in the significant bin. ', ... 
        newline, 'NOTE TO SELF to check this function ' ];
   
disp(notes)
   
  
mat_size=size(pa(:,:,1));
bin_event_matrix=[bins_event_tble events];
if iscell(mfull)
...
ct=1;
ct_last=1;
%this OBNOXIOUS line is the thing that kept printing the 2 
%figure out later if this 2 even has any purpose 
