...
%fmincon minimizes lij_optim_fun, initial point is sij1dy_init, lower
%bound and upperbound specified
...
% %        sij1d=[log_sij1d;0];
        ...
        %up to now, is same as before the optimization 
        ...
        %p_mult now lower triangular only
        p_mult=p_mult';
        %b_mat=1 for all intra tiles 
        p_vec=p_mult(logical(tril(b_mat)));

        %len factor calculates a len factor for all intra bin combinations 
        %(repeated 9 times based on length of d_sijdx)
